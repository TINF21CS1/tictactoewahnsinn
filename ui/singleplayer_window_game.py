...
class AI():
    ...
    def spielzug(self, board_status):
        ...
        if self.diff == "easy": # Schwere KI muss noch implementiert werden
            ...
            self._max_depth = 9
            arr = self.schwerer_spielzug(board_status)
            print(arr)
            return arr
    ...
    def schwerer_spielzug(self, board):
        best_eval = -float('inf')
        best_move = None
        ...
        for i in range(3):
            for j in range(3):
                if board[i][j] == 0:
                    board[i][j] = 1
                    eval = self.minimax(0, False, board)
                    board[i][j] = 0
                    if eval > best_eval:
                        best_eval = eval
                        best_move = [i, j]
        ...
        if is_maximizing:
            ...
            for i in range(3):
                for j in range(3):
                    if board[i][j] == 0:
                        board[i][j] = 1
                        eval = self.minimax(depth + 1, False, board)
                        board[i][j] = 0
            ...
            for i in range(3):
                for j in range(3):
                    if board[i][j] == 0:
                        board[i][j] = -1
                        eval = self.minimax(depth + 1, True, board)
                        board[i][j] = 0
